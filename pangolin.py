...
try:
    ...
    def supports(versions: List[int]) -> Callable:
        def g(f: Callable) -> Callable:
            @functools.wraps(f)
            def check_version(self: "Pangolin", *args: List, **kwargs: Dict) -> Any:
                if self.version not in versions:
                    raise Exception(
                        "Function does not support version passed to constructor"
                    )
    ...
    class Pangolin:
        def __init__(
            ...
            self.factory_contract = self._load_contract(
                abi_name="pang_factory", address=factory_contract_address,
            )
            ...
            self.router = self._load_contract(
                abi_name="pang_router", address=self.router_address,
            )
        ...
        @functools.lru_cache()
        @supports([2])
        def get_wavax_address(self) -> ChecksumAddress:
            # Contract calls should always return checksummed addresses
            address = Web3.toChecksumAddress('0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7')
            return address
        ...
        # ------ Market --------------------------------------------------------------------
        @supports([1, 2])
        def get_avax_token_input_price(self, token: AddressLike, qty: Wei) -> Wei:
            ...
            if self.version == 1:
                ...
                price = self.router.functions.getAmountsOut(
                    qty, [self.get_wavax_address(), token]
                ).call()[-1]
            return price

        @supports([1, 2])
        def get_token_avax_input_price(self, token: AddressLike, qty: int) -> int:
            ...
            if self.version == 1:
                ...
                price = self.router.functions.getAmountsOut(
                    qty, [token, self.get_wavax_address()]
                ).call()[-1]
            return price
        ...
        def get_token_token_input_price(
            ...
            if is_same_address(token0, self.get_wavax_address()):
                return int(self.get_avax_token_input_price(token1, qty))
            elif is_same_address(token1, self.get_wavax_address()):
                return int(self.get_token_avax_input_price(token0, qty))

            price: int = self.router.functions.getAmountsOut(
                qty, [token0, self.get_wavax_address(), token1]
            ).call()[-1]
            return price

        @supports([1, 2])
        def get_avax_token_output_price(self, token: AddressLike, qty: int) -> Wei:
        ...
        @supports([1, 2])
        def get_token_avax_output_price(self, token: AddressLike, qty: Wei) -> int:
        ...
        def get_token_token_output_price(
            ...
            # If one of the tokens are wavax, delegate to appropriate call.
            if is_same_address(token0, self.get_wavax_address()):
                return int(self.get_avax_token_output_price(token1, qty))
            elif is_same_address(token1, self.get_wavax_address()):
                return int(self.get_token_avax_output_price(token0, qty))
        ...
        # ------ ERC20 Pool ----------------------------------------------------------------
        @supports([1])
        def get_ex_avax_balance(self, token: AddressLike) -> int:
        ...
        def get_exchange_rate(self, token: AddressLike) -> float:
            ...
            avax_reserve = self.get_ex_avax_balance(token)
            token_reserve = self.get_ex_token_balance(token)
            return float(token_reserve / avax_reserve)

        # ------ Liquidity -----------------------------------------------------------------
        @supports([1])
        @check_approval
        def add_liquidity(
            self,gwei, my_address,my_pk, token: AddressLike, max_avax: Wei, min_liquidity: int = 1
        ) -> HexBytes:
            """Add liquidity to the pool."""
            tx_params = self._get_tx_params(value=max_avax, gwei=gwei)
            # Add 1 to avoid rounding errors, per
            # https://hackmd.io/hthz9hXKQmSyXfMbPsut1g#Add-Liquidity-Calculations
            max_token = int(max_avax * self.get_exchange_rate(token)) + 10
        ...
        @supports([1])
        @check_approval
        def remove_liquidity(self, gwei, my_address, my_pk, token: str, max_token: int) -> HexBytes:
        ...
        def make_trade(
            ...
            if input_token == ETH_ADDRESS:
                return self._avax_to_token_swap_input(gwei,my_address,my_pk,output_token, Wei(qty), recipient)
            else:
                balance = self.get_token_balance(input_token)
                if balance < qty:
                    raise InsufficientBalance(balance, qty)
                if output_token == ETH_ADDRESS:
                    return self._token_to_avax_swap_input(gwei,my_address,my_pk,input_token, qty, recipient)
                else:
        ...
        def make_trade_output(
            ...
            if input_token == ETH_ADDRESS:
                ...
                need = self.get_avax_token_output_price(output_token, qty)
                if balance < need:
                    raise InsufficientBalance(balance, need)
                return self._avax_to_token_swap_output(output_token, qty, recipient)
            else:
                if output_token == ETH_ADDRESS:
                    qty = Wei(qty)
                    return self._token_to_avax_swap_output(input_token, qty, recipient)
                else:
        ...
        def _avax_to_token_swap_input(
            ...
            avax_balance = self.get_avax_balance()
            if qty > avax_balance:
                raise InsufficientBalance(avax_balance, qty)
            ...
            if self.version == 1:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapExactETHForTokens(
                        amount_out_min,
                        [self.get_wavax_address(), output_token],
                        recipient,
                        self._deadline(),
                    ),
        ...
        def _token_to_avax_swap_input(
            ...
            if self.version == 1:
                ...
                amount_out_min = int(
                    (1 - self.max_slippage)
                    * self.get_token_avax_input_price(input_token, qty)
                )
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapExactTokensForETH(
                        qty,
                        amount_out_min,
                        [input_token, self.get_wavax_address()],
                        recipient,
                        self._deadline(),
                    ),
                )
        ...
        def _token_to_token_swap_input(
            ...
            if self.version == 1:
                ...
                min_tokens_bought, min_avax_bought = self._calculate_max_input_token(
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    self.router.functions.swapExactTokensForTokens(
                        qty,
                        min_tokens_bought,
                        [input_token, self.get_wavax_address(), output_token],
                        recipient,
                        self._deadline(),
                    ),
                )

        def _avax_to_token_swap_output(
            ...
            if self.version == 1:
                token_funcs = self.exchange_contract(output_token).functions
                avax_qty = self.get_avax_token_output_price(output_token, qty)
                tx_params = self._get_tx_params(value=avax_qty, gwei=gwei)
                func_params: List[Any] = [qty, self._deadline()]
                if not recipient:
                ...
                return self._build_and_send_tx(gwei, my_address,my_pk,
                    ...
                    self._get_tx_params(value=avax_qty, gwei=gwei),
                )

        def _token_to_avax_swap_output(
            ...
            # Balance check
            input_balance = self.get_token_balance(input_token)
            cost = self.get_token_avax_output_price(input_token, qty)
            ...
            if self.version == 1:
                ...
                outputReserve = self.get_ex_avax_balance(input_token)
        ...
        def _token_to_token_swap_output(
            ...
            if self.version == 1:
                token_funcs = self.exchange_contract(input_token).functions
                max_tokens_sold, max_avax_sold = self._calculate_max_input_token(
                    input_token, qty, output_token
                )
        ...
        def _calculate_max_input_token(
            ...
            """
            # Buy TokenB with AVAX
            output_amount_b = qty
            input_reserve_b = self.get_ex_avax_balance(output_token)
        ...
        def _calculate_max_output_token(
            ...
            outputReserveB = self.get_ex_avax_balance(output_token)
